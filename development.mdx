...
1. **Option 1: Add `.nvmrc` file** (Recommended)
...
2. **Option 2: Specify in `package.json`**
...
3. **Option 3: Set in Railway Dashboard**
...
<Accordion title="Issue: Encountering an unknown error">
  ...
  <Accordion title='Error: Invalid regular expression flags (Railway deployment)'>
    ...
    **Solution:**
    1. Create a `.nvmrc` file in your docs directory with `20` (or `19`)
    2. Or add `"engines": { "node": ">=19.0.0" }` to your `package.json`
    3. Or set `NODE_VERSION=20` in Railway environment variables
