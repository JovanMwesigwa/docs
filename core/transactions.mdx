...
<Steps>
  <Step title="Created">
    Transaction is created and validated by FundKit
  ...
  <Step title="Submitted">
    Transaction is sent to the selected payment provider
  ...
  <Step title="Processing">
    Provider processes the transaction with the mobile network
...
// Success rates
const successRate = transactions.filter(tx => tx.status === 'completed').length / transactions.length;
